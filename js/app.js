...
// ============================================================
// OPENING VOICE NARRATION (MALE SPEECH SYNTHESIS)
// ============================================================
...
function executeWelcomeVoice(force = false) {
    ...
    try {
        ...
        const text = "Welcome to POS Haltea Indonesia";
        ...
        const maleVoice = voices.find(v => {
            ...
            return (n.includes('male') || n.includes('david') || n.includes('george') || n.includes('guy') || n.includes('richard') || n.includes('ardhi') || n.includes('damon') || n.includes('natural')) && (v.lang.startsWith('en') || v.lang.startsWith('id'));
        }) || voices.find(v => v.lang.startsWith('en') || v.lang.startsWith('id')) || voices[0];

        if (maleVoice) utter.voice = maleVoice;
        utter.rate = 0.92;
        utter.pitch = 0.82; // Lower pitch ensures masculine voice
        utter.volume = 1;
...
// ============================================================
// HELPER: ESTIMASI PEMAKAIAN BAHAN BAKU PER MENU
// ============================================================
function getBahanUsageSummary(id_menu, total_cups) {
    if (!total_cups || total_cups <= 0) return `<span class="text-gray-400 text-[11px]">-</span>`;
    
    const takaranList = (typeof getMockStorage === 'function' ? getMockStorage('takaran', DEFAULT_TAKARAN) : DEFAULT_TAKARAN) || [];
    const barangList = (allBarang && allBarang.length > 0) ? allBarang : (typeof getMockStorage === 'function' ? getMockStorage('barang', DEFAULT_BARANG) : (window.DEFAULT_BARANG || []));

    const recipe = takaranList.filter(t => t.id_menu === id_menu || parseInt(t.id_menu, 10) === parseInt(id_menu, 10));
    if (!recipe || recipe.length === 0) {
        const totalTeh = 20 * total_cups;
        const totalGula = 25 * total_cups;
        return `
        <div class="flex flex-wrap gap-1 items-center">
            <span class="px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-[10px] font-semibold">Teh ${totalTeh.toLocaleString('id-ID')}g</span>
            <span class="px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-[10px] font-semibold">Gula ${totalGula.toLocaleString('id-ID')}g</span>
            <span class="px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-[10px] font-semibold">${total_cups.toLocaleString('id-ID')} Cup</span>
        </div>`;
    }

    const badges = recipe.map(r => {
        const b = barangList.find(x => x.id === r.id_barang || parseInt(x.id, 10) === parseInt(r.id_barang, 10));
        const bName = b ? b.nama_barang.replace(/^(Bahan\s*Baku\s*|Haltea\s*)/i, '').trim() : `Bahan #${r.id_barang}`;
        const satuan = b?.satuan || 'gr';
        const totalGram = (Number(r.gramasi) || 0) * total_cups;
        let shortName = bName;
        ...
        if (satuan === 'pcs') colorClass = 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300';
        else if (bName.toLowerCase().includes('teh') || bName.toLowerCase().includes('tea')) colorClass = 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300';
        else if (bName.toLowerCase().includes('gula')) colorClass = 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300';
        else if (bName.toLowerCase().includes('susu') || bName.toLowerCase().includes('milk')) colorClass = 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300';
        else if (bName.toLowerCase().includes('sirup') || bName.toLowerCase().includes('flavour')) colorClass = 'bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300';

        return `<span class="px-1.5 py-0.5 rounded ${colorClass} text-[10px] font-semibold">${shortName}: ${totalGram.toLocaleString('id-ID')}${satuan}</span>`;
    });
...
// ============================================================
// MODAL RIWAYAT TRANSAKSI HARI INI (KASIR POS EDIT & HAPUS)
// ============================================================
let posModalHistoryData = [];
let posModalSearchFilter = '';

async function openModalRiwayatHariIni() {
    const today = new Date().toISOString().split('T')[0];
    const dateLabel = document.getElementById('pos-modal-date-label');
    if (dateLabel) {
        const d = new Date();
        const opts = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
        dateLabel.textContent = d.toLocaleDateString('id-ID', opts) + ` (${today})`;
    }
    ...
    try {
        ...
        const res = await apiFetch(`/api/transaksi?tanggal=${today}&limit=500`);
        if (res && res.ok) {
            posModalHistoryData = await res.json();
        } else {
            const allTrx = typeof getMockStorage === 'function' ? getMockStorage('transaksi', DEFAULT_TRANSAKSI) : (window.DEFAULT_TRANSAKSI || []);
            posModalHistoryData = allTrx.filter(t => t.tanggal === today);
        }
    } catch (e) {
        const allTrx = typeof getMockStorage === 'function' ? getMockStorage('transaksi', DEFAULT_TRANSAKSI) : (window.DEFAULT_TRANSAKSI || []);
        posModalHistoryData = allTrx.filter(t => t.tanggal === today);
    ...
    renderPosModalHistory();
    openModal('modal-riwayat-hari-ini');
}
window.openModalRiwayatHariIni = openModalRiwayatHariIni;

function filterPosModalHistory(val) {
    posModalSearchFilter = (val || '').toLowerCase().trim();
    renderPosModalHistory();
}
window.filterPosModalHistory = filterPosModalHistory;

function renderPosModalHistory() {
    const container = document.getElementById('pos-modal-history-list');
    if (!container) return;

    let filtered = (posModalHistoryData || []);
    if (posModalSearchFilter) {
        filtered = filtered.filter(t => (t.nama_menu || '').toLowerCase().includes(posModalSearchFilter));
    }

    // Badges calculation
    const totalTrx = posModalHistoryData.length;
    const totalCups = posModalHistoryData.reduce((s, t) => s + (parseInt(t.jumlah) || 0), 0);
    const totalOmset = posModalHistoryData.reduce((s, t) => s + (parseInt(t.total_harga || t.total_bayar) || (parseInt(t.harga) * parseInt(t.jumlah)) || 0), 0);

    const elCount = document.getElementById('pos-modal-trx-count');
    const elCups = document.getElementById('pos-modal-cup-count');
    const elOmset = document.getElementById('pos-modal-omset');
    if (elCount) elCount.textContent = `${totalTrx} Transaksi`;
    if (elCups) elCups.textContent = `${totalCups.toLocaleString('id-ID')} Cup`;
    if (elOmset) elOmset.textContent = 'Rp ' + formatNum(totalOmset, 0);

    if (filtered.length === 0) {
        container.innerHTML = `
        <div class="py-10 text-center text-gray-400 dark:text-gray-500">
            <i class="fas fa-receipt text-3xl mb-2 block text-gray-300 dark:text-gray-600"></i>
            <p class="text-xs font-semibold">Belum ada transaksi pada hari ini.</p>
        </div>`;
        return;
    }
    ...
    <table class="w-full data-table">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ${filtered.map((t, idx) => {
                ...
                const harga = parseInt(t.harga) || parseInt(m?.harga) || 5000;
                const total = parseInt(t.total_harga || t.total_bayar) || (harga * parseInt(t.jumlah || 1));
                return `
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition">
                    <td class="py-2.5 px-3 text-xs font-bold text-gray-400 text-center" data-label="No">${idx + 1}</td>
                    <td class="py-2.5 px-3 text-xs font-bold text-gray-900 dark:text-white td-title" data-label="Menu">
                        <div class="flex items-center gap-1.5">
                            <span class="w-2 h-2 rounded-full bg-red-500 shrink-0"></span>
                            <span>${menuName}</span>
                        </div>
                    </td>
                    <td class="py-2.5 px-3 text-xs font-black text-purple-600 dark:text-purple-400 text-center" data-label="Jumlah">${t.jumlah} Cup</td>
                    <td class="py-2.5 px-3 text-xs font-bold text-red-600 dark:text-red-400 text-right" data-label="Total">Rp ${formatNum(total, 0)}</td>
                    <td class="py-2.5 px-3 text-center td-actions" data-label="Aksi">
                        <div class="flex items-center justify-end gap-1.5">
                            <button onclick="openEditTransaksi(${t.id})" class="px-2.5 py-1 rounded-lg bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-600 dark:text-blue-400 text-xs font-semibold flex items-center gap-1 transition">
                                <i class="fas fa-edit text-xs"></i> <span>Edit</span>
                            </button>
                            <button onclick="deleteTransaksiPOS(${t.id})" class="px-2.5 py-1 rounded-lg bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 text-xs font-semibold flex items-center gap-1 transition">
...
async function deleteTransaksiPOS(id) {
    showConfirm('Hapus transaksi ini? Stok bahan baku akan dikembalikan.', async () => {
        try {
            await apiFetch(`/api/transaksi/${id}`, { method: 'DELETE' });
            showToast('Transaksi dihapus, stok dikembalikan.', 'success');
            await openModalRiwayatHariIni();
            await loadTransaksiCatalog();
...
window.deleteTransaksiPOS = deleteTransaksiPOS;

// Legacy support for today-history container
...
async function deleteTodayTrx(id) {
    await deleteTransaksiPOS(id);
}

// ============================================================
// RIWAYAT TRANSAKSI (4 TABS: HARIAN, 1 PEKAN, 1 BULAN, TAHUNAN)
// ============================================================
...
function switchRiwayatTab(tab) {
    ...
    const tabs = ['per_transaksi', 'pekan', 'bulan', 'tahun'];
    tabs.forEach(t => {
...
async function loadRiwayatTransaksi() {
    ...
    if (!allBarang || allBarang.length === 0) {
        try {
            const bRes = await apiFetch('/api/barang');
            if (bRes && bRes.ok) allBarang = await bRes.json();
        } catch (e) {}
    }
    ...
    } else if (currentRiwayatTab === 'tahun') {
        renderRiwayatTahun(allTrx);
    }
}
...
function setRiwayatDateToday() {
    const dateInput = document.getElementById('riwayat-filter-date');
    const today = new Date().toISOString().slice(0, 10);
    if (dateInput) dateInput.value = today;
    loadRiwayatTransaksi();
}
window.setRiwayatDateToday = setRiwayatDateToday;

// Helper: group transactions by menu and calculate sum & stats
function groupTrxByMenu(trxs) {
    const map = {};
    ...
    (trxs || []).forEach((t, idx) => {
        ...
        map[menuId].total_cups += jml;
        map[menuId].total_omset += total;
        map[menuId].trx_count += 1;

        grandTotalOmset += total;
        grandTotalCups += jml;
    });

    const list = Object.values(map).sort((a, b) => b.total_cups - a.total_cups || b.total_omset - a.total_omset);
    return { list, grandTotalOmset, grandTotalCups };
}

// -------------------------------------------------------------
// TAB 1: HARIAN (REKAP PENJUALAN PER MENU & DETAIL TRANSAKSI)
// -------------------------------------------------------------
function renderRiwayatPerTransaksi(allTrx) {
    ...
    const elStatDate = document.getElementById('harian-stat-date-label');
    const elStatCups = document.getElementById('harian-stat-cups');
    const elStatTrx = document.getElementById('harian-stat-trx');
    const elStatMenuCount = document.getElementById('harian-stat-menu-count');
    const elStatTopMenu = document.getElementById('harian-stat-top-menu');
    const badge = document.getElementById('riwayat-total-count-badge');

    if (elStatOmset) elStatOmset.textContent = 'Rp ' + formatNum(totalOmset, 0);
    if (elStatDate) {
        elStatDate.textContent = filterDate ? (filterDate === todayIso ? 'Hari ini (' + filterDate + ')' : 'Tanggal ' + filterDate) : 'Semua Riwayat Terdata';
    }
    if (elStatCups) elStatCups.textContent = `${totalCups.toLocaleString('id-ID')} Cup`;
    if (elStatTrx) elStatTrx.textContent = `${filtered.length} kali transaksi`;
    if (elStatMenuCount) elStatMenuCount.textContent = `${menuBreakdown.length} Menu Terjual`;
    if (elStatTopMenu) {
        elStatTopMenu.textContent = menuBreakdown.length > 0 ? `Top: ${menuBreakdown[0].nama_menu} (${menuBreakdown[0].total_cups} cup)` : 'Top: Belum ada penjualan';
    }
    if (badge) badge.textContent = `${filtered.length} Transaksi`;

    // 2. Render Tabel Rekap Penjualan Per Menu Harian (Murni Penjualan)
        ...
        if (menuBreakdown.length === 0) {
            ...
            tbodyMenu.innerHTML = menuBreakdown.map((item, idx) => {
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                    <td class="px-3 py-2.5 text-xs font-semibold text-gray-400 text-center" data-label="No">${idx + 1}</td>
                    <td class="px-3 py-2.5 text-xs font-bold text-gray-900 dark:text-white td-title" data-label="Nama Menu">
                        <div class="flex items-center gap-2">
                            <span class="w-5 h-5 rounded-md bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 font-bold text-[10px] flex items-center justify-center">${idx + 1}</span>
                            <span>${item.nama_menu}</span>
                        </div>
                    </td>
                    <td class="px-3 py-2.5 text-xs text-gray-500 dark:text-gray-400" data-label="Kategori">
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300">${item.kategori}</span>
                    </td>
                    <td class="px-3 py-2.5 text-xs font-extrabold text-center text-purple-600 dark:text-purple-400" data-label="Jumlah Penjualan">${item.total_cups.toLocaleString('id-ID')} Cup</td>
                    <td class="px-3 py-2.5 text-xs font-bold text-right text-red-600 dark:text-red-400" data-label="Total Omset">Rp ${formatNum(item.total_omset, 0)}</td>
                    <td class="px-3 py-2.5 text-xs text-center" data-label="Kontribusi">
                        <div class="flex items-center justify-center gap-1.5">
...
// -------------------------------------------------------------
// TAB 2: PEKANAN (7 KOLOM UNTUK 7 HARI MATRIX PER MENU)
// -------------------------------------------------------------
function presetPekanFilter(type) {
    const fromEl = document.getElementById('pekan-filter-from');
    const toEl = document.getElementById('pekan-filter-to');
    const now = new Date();

    if (type === '7days') {
        const fromD = new Date(now);
        fromD.setDate(fromD.getDate() - 6);
        if (fromEl) fromEl.value = fromD.toISOString().slice(0, 10);
        ...
        if (fromEl) fromEl.value = monday.toISOString().slice(0, 10);
        if (toEl) toEl.value = now.toISOString().slice(0, 10);
    }
    loadRiwayatPekanCustom();
}
window.presetPekanFilter = presetPekanFilter;

function loadRiwayatPekanCustom() {
    loadRiwayatTransaksi();
}
window.loadRiwayatPekanCustom = loadRiwayatPekanCustom;
...
function renderRiwayatPekan(allTrx) {
    ...
    // Generate 7 consecutive days list
    const daysArray = [];
    ...
    if (fromDate && toDate) {
        ...
        let safetyCount = 0;
        while (cur <= end && safetyCount < 31) {
            const iso = cur.toISOString().slice(0, 10);
            const dName = dayShortNames[cur.getDay()];
            const dDate = `${cur.getDate()}/${cur.getMonth() + 1}`;
            daysArray.push({ iso, label: `${dName} (${dDate})`, short: `${dName} ${cur.getDate()}` });
            cur.setDate(cur.getDate() + 1);
            safetyCount++;
        }
    }
    // If not 7 days, fallback to 7 days up to toDate
    if (daysArray.length === 0) {
        for (let i = 6; i >= 0; i--) {
            const d = new Date(now);
            d.setDate(d.getDate() - i);
            const iso = d.toISOString().slice(0, 10);
            const dName = dayShortNames[d.getDay()];
            const dDate = `${d.getDate()}/${d.getMonth() + 1}`;
            daysArray.push({ iso, label: `${dName} (${dDate})`, short: `${dName} ${d.getDate()}` });
        }
    }

    // Filter transactions in date range
    const filteredTrxs = (allTrx || []).filter(t => {
        if (!t.tanggal) return false;
        if (fromDate && t.tanggal < fromDate) return false;
        if (toDate && t.tanggal > toDate) return false;
        return true;
    });

    // 1. KPI Cards
    const totalPekanOmset = filteredTrxs.reduce((s, t) => s + (parseInt(t.total_harga || t.total_bayar) || (parseInt(t.harga) * parseInt(t.jumlah || 1)) || 0), 0);
    const totalPekanCups = filteredTrxs.reduce((s, t) => s + (parseInt(t.jumlah) || 0), 0);
    const dayCount = Math.max(1, daysArray.length);
    const avgDailyOmset = Math.round(totalPekanOmset / dayCount);
    const avgDailyCups = Math.round(totalPekanCups / dayCount);
    ...
    const elPekanSummary = document.getElementById('pekan-summary-range-label');
    ...
    if (elPekanTrx) elPekanTrx.textContent = `${filteredTrxs.length} kali transaksi`;
    if (elPekanAvg) elPekanAvg.textContent = 'Rp ' + formatNum(avgDailyOmset, 0) + ' / hari';
    if (elPekanAvgCups) elPekanAvgCups.textContent = `${avgDailyCups} cup / hari (${dayCount} hari)`;
    if (elPekanDateRange) elPekanDateRange.textContent = `${fromDate} s/d ${toDate}`;
    if (elPekanSummary) elPekanSummary.textContent = `Periode: ${fromDate} s/d ${toDate}`;

    // 2. Dynamic Update Table Header for 7 Days Matrix
    const theadTr = document.getElementById('table-riwayat-menu-pekan-head-tr');
    if (theadTr) {
        theadTr.innerHTML = `
            <th class="px-2.5 py-2.5 text-center font-semibold w-10">No</th>
            <th class="px-3 py-2.5 text-left font-semibold">Nama Menu</th>
            ${daysArray.map(d => `<th class="px-2 py-2.5 text-center font-semibold whitespace-nowrap">${d.label}</th>`).join('')}
            <th class="px-2.5 py-2.5 text-center font-semibold text-purple-600 dark:text-purple-400">Total Cup</th>
            <th class="px-3 py-2.5 text-right font-semibold text-red-600 dark:text-red-400">Total Omset</th>
            <th class="px-2.5 py-2.5 text-center font-semibold">Rata-rata/Hari</th>`;
    }

    // 3. Rekapitulasi Penjualan Per Menu (7 Hari Matrix)
    // Structure: map menuId => { id_menu, nama_menu, harga, dailyCups: { [iso]: cups }, totalCups, totalOmset }
    const menuMatrixMap = {};
    
    // Seed with all catalog menus so all menu variants are visible
    (allMenu || []).forEach(m => {
        menuMatrixMap[m.id] = {
            id_menu: m.id,
            nama_menu: m.nama_menu,
            kategori: m.kategori || 'Minuman',
            harga: parseInt(m.harga) || 5000,
            dailyCups: {},
            totalCups: 0,
            totalOmset: 0
        };
    ...
    filteredTrxs.forEach((t, idx) => {
        let m = allMenu.find(x => x.id === t.id_menu || parseInt(x.id, 10) === parseInt(t.id_menu, 10));
        if (!m && t.nama_menu && t.nama_menu !== 'Menu Varian' && t.nama_menu !== 'Menu Lainnya') {
            m = allMenu.find(x => (x.nama_menu || '').toLowerCase() === (t.nama_menu || '').toLowerCase());
        }
        const menuId = m?.id || t.id_menu || `custom_${idx}`;
        const menuName = (t.nama_menu && t.nama_menu !== 'Menu Varian' && t.nama_menu !== 'Menu Lainnya') ? t.nama_menu : (m?.nama_menu || 'Teh Manis Original');
        const harga = parseInt(t.harga) || parseInt(m?.harga) || 5000;
        const jml = parseInt(t.jumlah) || 1;
        const total = parseInt(t.total_harga || t.total_bayar) || (harga * jml);

        if (!menuMatrixMap[menuId]) {
            menuMatrixMap[menuId] = {
                id_menu: menuId,
                nama_menu: menuName,
                kategori: t.kategori || m?.kategori || 'Minuman',
                harga: harga,
                dailyCups: {},
                totalCups: 0,
                totalOmset: 0
        ...
        const tgl = t.tanggal;
        menuMatrixMap[menuId].dailyCups[tgl] = (menuMatrixMap[menuId].dailyCups[tgl] || 0) + jml;
        menuMatrixMap[menuId].totalCups += jml;
        menuMatrixMap[menuId].totalOmset += total;
    });

    const matrixList = Object.values(menuMatrixMap)
        .filter(m => m.totalCups > 0 || (allMenu && allMenu.length > 0)) // show all menu items or active ones
        .sort((a, b) => b.totalCups - a.totalCups || b.totalOmset - a.totalOmset);

    const tbodyMenuPekan = document.getElementById('table-riwayat-menu-pekan-body');
    if (tbodyMenuPekan) {
        if (matrixList.length === 0 || totalPekanCups === 0) {
            tbodyMenuPekan.innerHTML = `<tr><td colspan="${daysArray.length + 5}" class="text-center py-8 text-gray-400 dark:text-gray-600 text-xs">Belum ada transaksi dalam rentang 7 hari ini.</td></tr>`;
        } else {
            tbodyMenuPekan.innerHTML = matrixList.map((m, idx) => {
                const avgMenuCup = (m.totalCups / dayCount).toFixed(1);

                const dayCells = daysArray.map(d => {
                    const cups = m.dailyCups[d.iso] || 0;
                    if (cups > 0) {
                        return `<td class="px-2 py-2 text-center text-xs font-bold text-gray-900 dark:text-white bg-red-50/20 dark:bg-red-900/10" data-label="${d.short}">${cups}</td>`;
                    }
                    return `<td class="px-2 py-2 text-center text-xs text-gray-300 dark:text-gray-600" data-label="${d.short}">-</td>`;
                }).join('');

                return `
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                    <td class="px-2.5 py-2.5 text-center text-xs font-bold text-gray-400" data-label="No">${idx + 1}</td>
                    <td class="px-3 py-2.5 text-xs font-bold text-gray-900 dark:text-white td-title" data-label="Nama Menu">${m.nama_menu}</td>
                    ${dayCells}
                    <td class="px-2.5 py-2.5 text-xs text-center font-black text-purple-600 dark:text-purple-400" data-label="Total Cup">${m.totalCups.toLocaleString('id-ID')} Cup</td>
                    <td class="px-3 py-2.5 text-xs text-right font-bold text-red-600 dark:text-red-400" data-label="Total Omset">Rp ${formatNum(m.totalOmset, 0)}</td>
                    <td class="px-2.5 py-2.5 text-xs text-center text-gray-700 dark:text-gray-300 font-medium" data-label="Rata-rata/Hari">${avgMenuCup} cup/hari</td>
                </tr>`;
            }).join('');
        }
    }
...
// -------------------------------------------------------------
// TAB 3: BULANAN (4 PEKAN MATRIX PER MENU)
// -------------------------------------------------------------
function presetBulanFilter(type) {
    const monthInput = document.getElementById('riwayat-filter-month');
    const fromEl = document.getElementById('bulan-filter-from');
    const toEl = document.getElementById('bulan-filter-to');
    const now = new Date();
    ...
    if (type === 'thismonth') {
        ...
        if (monthInput) monthInput.value = lastM.toISOString().slice(0, 7);
    }
    loadRiwayatBulan();
}
window.presetBulanFilter = presetBulanFilter;

function loadRiwayatBulanCustomRange() {
    loadRiwayatTransaksi();
}
window.loadRiwayatBulanCustomRange = loadRiwayatBulanCustomRange;
...
function renderRiwayatBulan(allTrx) {
    ...
    if (customFrom || customTo) {
        trxsInMonth = (allTrx || []).filter(t => {
            if (!t.tanggal) return false;
            if (customFrom && t.tanggal < customFrom) return false;
            if (customTo && t.tanggal > customTo) return false;
            return true;
        });
        periodLabel = `Rentang ${customFrom || 'Awal'} s/d ${customTo || 'Akhir'}`;
    } else {
        if (monthInput && !monthInput.value) {
            monthInput.value = currentMonthIso;
        }
        selectedMonth = monthInput?.value || currentMonthIso;
        trxsInMonth = (allTrx || []).filter(t => t.tanggal && t.tanggal.startsWith(selectedMonth));
        periodLabel = `Periode Bulan ${selectedMonth}`;
    }

    // 1. Group by date for KPI & daily table
    const dailyMap = {};
    trxsInMonth.forEach(t => {
        ...
        dailyMap[d].omset += (parseInt(t.total_harga || t.total_bayar) || (parseInt(t.harga) * parseInt(t.jumlah)) || 0);
    });

    const datesSorted = Object.keys(dailyMap).sort((a, b) => b.localeCompare(a));
    const totalMonthOmset = trxsInMonth.reduce((sum, t) => sum + (parseInt(t.total_harga || t.total_bayar) || (parseInt(t.harga) * parseInt(t.jumlah)) || 0), 0);
    ...
    if (elBulanLabel) elBulanLabel.textContent = `Rekapan: ${periodLabel}`;

    // 3. Rekapitulasi Penjualan Per Menu (4 Pekan Matrix: Tgl 1-7, 8-14, 15-21, 22-End)
    ...
    trxsInMonth.forEach((t, idx) => {
        ...
        if (!monthMatrixMap[menuId]) {
            monthMatrixMap[menuId] = {
                id_menu: menuId,
                nama_menu: menuName,
                kategori: t.kategori || m?.kategori || 'Minuman',
                harga: harga,
        ...
        // Determine which week: 1-7 (w1), 8-14 (w2), 15-21 (w3), 22+ (w4)
        const dayNum = parseInt((t.tanggal || '').slice(8, 10), 10) || 1;
        if (dayNum <= 7) monthMatrixMap[menuId].w1 += jml;
        else if (dayNum <= 14) monthMatrixMap[menuId].w2 += jml;
        else if (dayNum <= 21) monthMatrixMap[menuId].w3 += jml;
        else monthMatrixMap[menuId].w4 += jml;

        monthMatrixMap[menuId].totalCups += jml;
        monthMatrixMap[menuId].totalOmset += total;
    });

    const monthList = Object.values(monthMatrixMap)
        .filter(m => m.totalCups > 0 || (allMenu && allMenu.length > 0))
        .sort((a, b) => b.totalCups - a.totalCups || b.totalOmset - a.totalOmset);

    const tbodyMenuBulan = document.getElementById('table-riwayat-menu-bulan-body');
    if (tbodyMenuBulan) {
        if (monthList.length === 0 || totalMonthCups === 0) {
            tbodyMenuBulan.innerHTML = `<tr><td colspan="9" class="text-center py-8 text-gray-400 dark:text-gray-600 text-xs">Belum ada penjualan menu pada periode bulan ini.</td></tr>`;
        } else {
            tbodyMenuBulan.innerHTML = monthList.map((m, idx) => {
                const avgWeekCup = (m.totalCups / 4).toFixed(1);
                return `
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                    <td class="px-3 py-2.5 text-center text-xs font-bold text-gray-400" data-label="No">${idx + 1}</td>
                    <td class="px-4 py-2.5 text-xs font-bold text-gray-900 dark:text-white td-title" data-label="Nama Menu">${m.nama_menu}</td>
                    <td class="px-3 py-2.5 text-center text-xs font-bold text-gray-800 dark:text-gray-200" data-label="Pekan 1">${m.w1 > 0 ? m.w1 : `<span class="text-gray-300 dark:text-gray-600">-</span>`}</td>
                    <td class="px-3 py-2.5 text-center text-xs font-bold text-gray-800 dark:text-gray-200" data-label="Pekan 2">${m.w2 > 0 ? m.w2 : `<span class="text-gray-300 dark:text-gray-600">-</span>`}</td>
                    <td class="px-3 py-2.5 text-center text-xs font-bold text-gray-800 dark:text-gray-200" data-label="Pekan 3">${m.w3 > 0 ? m.w3 : `<span class="text-gray-300 dark:text-gray-600">-</span>`}</td>
                    <td class="px-3 py-2.5 text-center text-xs font-bold text-gray-800 dark:text-gray-200" data-label="Pekan 4">${m.w4 > 0 ? m.w4 : `<span class="text-gray-300 dark:text-gray-600">-</span>`}</td>
                    <td class="px-3 py-2.5 text-center font-black text-purple-600 dark:text-purple-400 text-xs" data-label="Total Cup">${m.totalCups.toLocaleString('id-ID')} Cup</td>
                    <td class="px-4 py-2.5 text-right font-bold text-red-600 dark:text-red-400 text-xs" data-label="Total Omset">Rp ${formatNum(m.totalOmset, 0)}</td>
                    <td class="px-3 py-2.5 text-center text-gray-700 dark:text-gray-300 font-medium text-xs" data-label="Rata-rata/Pekan">${avgWeekCup} cup/pekan</td>
                </tr>`;
            }).join('');
        }
    }
...
// -------------------------------------------------------------
// TAB 4: TAHUNAN (12 BULAN MATRIX PER MENU)
// -------------------------------------------------------------
function setRiwayatYearCurrent() {
    const select = document.getElementById('riwayat-filter-year');
    const curY = new Date().getFullYear();
    if (select) select.value = String(curY);
    loadRiwayatTahun();
}
window.setRiwayatYearCurrent = setRiwayatYearCurrent;

function loadRiwayatTahun() {
    loadRiwayatTransaksi();
}
window.loadRiwayatTahun = loadRiwayatTahun;

function renderRiwayatTahun(allTrx) {
    const yearSelect = document.getElementById('riwayat-filter-year');
    const selectedYear = yearSelect?.value || String(new Date().getFullYear());

    const yearSummary = document.getElementById('tahun-summary-label');
    if (yearSummary) yearSummary.textContent = `Periode Tahun ${selectedYear} (01 Jan - 31 Des ${selectedYear})`;

    const trxsInYear = (allTrx || []).filter(t => t.tanggal && t.tanggal.startsWith(selectedYear));

    // 1. KPI Calculation
    const totalYearOmset = trxsInYear.reduce((s, t) => s + (parseInt(t.total_harga || t.total_bayar) || (parseInt(t.harga) * parseInt(t.jumlah)) || 0), 0);
    const totalYearCups = trxsInYear.reduce((s, t) => s + (parseInt(t.jumlah) || 0), 0);
    
    const activeMonthsSet = new Set(trxsInYear.map(t => t.tanggal.slice(0, 7)));
    const activeMonthCount = Math.max(1, activeMonthsSet.size);
    const avgMonthlyOmset = Math.round(totalYearOmset / activeMonthCount);

    const { list: menuYearList } = groupTrxByMenu(trxsInYear);
    const topMenu = menuYearList.length > 0 ? menuYearList[0] : null;

    const elStatOmset = document.getElementById('tahun-stat-omset');
    const elStatYearLabel = document.getElementById('tahun-stat-year-label');
    const elStatCups = document.getElementById('tahun-stat-cups');
    const elStatTrx = document.getElementById('tahun-stat-trx-count');
    const elStatAvg = document.getElementById('tahun-stat-avg-month');
    const elStatTopMenu = document.getElementById('tahun-stat-top-menu');
    const elStatTopDesc = document.getElementById('tahun-stat-top-desc');

    if (elStatOmset) elStatOmset.textContent = 'Rp ' + formatNum(totalYearOmset, 0);
    if (elStatYearLabel) elStatYearLabel.textContent = `Tahun ${selectedYear} (${activeMonthCount} bln aktif)`;
    if (elStatCups) elStatCups.textContent = `${totalYearCups.toLocaleString('id-ID')} Cup`;
    if (elStatTrx) elStatTrx.textContent = `${trxsInYear.length} kali transaksi`;
    if (elStatAvg) elStatAvg.textContent = 'Rp ' + formatNum(avgMonthlyOmset, 0);
    if (elStatTopMenu) elStatTopMenu.textContent = topMenu ? topMenu.nama_menu : '-';
    if (elStatTopDesc) elStatTopDesc.textContent = topMenu ? `${topMenu.total_cups} Cup (Rp ${formatNum(topMenu.total_omset, 0)})` : 'Belum ada penjualan';

    // 2. Table 1: Rekapitulasi Penjualan Per Menu (12 Bulan Matrix: Jan - Des)
    const yearMatrixMap = {};
    (allMenu || []).forEach(m => {
        yearMatrixMap[m.id] = {
            id_menu: m.id,
            ...
            kategori: m.kategori || 'Minuman',
            harga: parseInt(m.harga) || 5000,
            months: new Array(12).fill(0), // Jan=0, Feb=1, ..., Des=11
            totalCups: 0,
            totalOmset: 0
        };
    });

    trxsInYear.forEach((t, idx) => {
        let m = allMenu.find(x => x.id === t.id_menu || parseInt(x.id, 10) === parseInt(t.id_menu, 10));
        if (!m && t.nama_menu && t.nama_menu !== 'Menu Varian' && t.nama_menu !== 'Menu Lainnya') {
            m = allMenu.find(x => (x.nama_menu || '').toLowerCase() === (t.nama_menu || '').toLowerCase());
        }
        ...
        if (!yearMatrixMap[menuId]) {
            yearMatrixMap[menuId] = {
                id_menu: menuId,
                nama_menu: menuName,
                kategori: t.kategori || m?.kategori || 'Minuman',
                harga: harga,
        ...
        const mIdx = parseInt((t.tanggal || '').slice(5, 7), 10) - 1;
        if (mIdx >= 0 && mIdx < 12) {
            yearMatrixMap[menuId].months[mIdx] += jml;
        }

        yearMatrixMap[menuId].totalCups += jml;
        yearMatrixMap[menuId].totalOmset += total;
    });

    const yearList = Object.values(yearMatrixMap)
        .filter(m => m.totalCups > 0 || (allMenu && allMenu.length > 0))
        .sort((a, b) => b.totalCups - a.totalCups || b.totalOmset - a.totalOmset);

    const tbodyMenuYear = document.getElementById('table-riwayat-menu-tahun-body');
    if (tbodyMenuYear) {
        if (yearList.length === 0 || totalYearCups === 0) {
            tbodyMenuYear.innerHTML = `<tr><td colspan="17" class="text-center py-8 text-gray-400 text-xs">Belum ada transaksi di tahun ${selectedYear}.</td></tr>`;
        } else {
            const shortMonthLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
            tbodyMenuYear.innerHTML = yearList.map((m, idx) => {
                const avgMonthCup = (m.totalCups / 12).toFixed(1);

                const monthCells = m.months.map((cups, mIndex) => {
                    if (cups > 0) {
                        return `<td class="px-1.5 py-2 text-center text-xs font-bold text-gray-900 dark:text-white" data-label="${shortMonthLabels[mIndex]}">${cups}</td>`;
                    }
                    return `<td class="px-1.5 py-2 text-center text-xs text-gray-300 dark:text-gray-600" data-label="${shortMonthLabels[mIndex]}">-</td>`;
                }).join('');

                return `
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                    <td class="px-2 py-2 text-center text-xs font-bold text-gray-400" data-label="No">${idx + 1}</td>
                    <td class="px-3 py-2 text-xs font-bold text-gray-900 dark:text-white td-title" data-label="Nama Menu">${m.nama_menu}</td>
                    ${monthCells}
                    <td class="px-2.5 py-2 text-xs text-center font-black text-purple-600 dark:text-purple-400" data-label="Total Cup">${m.totalCups.toLocaleString('id-ID')} Cup</td>
                    <td class="px-3 py-2 text-xs text-right font-bold text-red-600 dark:text-red-400" data-label="Total Omset">Rp ${formatNum(m.totalOmset, 0)}</td>
                    <td class="px-2.5 py-2 text-xs text-center text-gray-700 dark:text-gray-300 font-medium" data-label="Rata-rata/Bulan">${avgMonthCup} cup/bln</td>
                </tr>`;
            }).join('');
        }
    }
...
async function submitEditTransaksi(e) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            await loadRiwayatPage();
            await loadTransaksiCatalog();
            const posModal = document.getElementById('modal-riwayat-hari-ini');
